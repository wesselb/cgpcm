...
\begin{figure*}[t]
    ...
    \caption{Toy. Here {\color{red}red} denotes truth; {\color{darkgreen}green} denotes observations, the emperical autocorrelation, and the periodogram for respectively the function $f\cond h$, kernel $k_{f\cond h}$, and PSD of $k_{f\cond h}$; {\color{blue}blue} denotes quantities learned through the MF approximation; and {\color{mycyan}cyan} denotes quantities learned through the SMF approximation. Signals normalised to unity power.}
    \label{fig:toy_acausal_sample}
\end{figure*}

Check out \cref{fig:toy_causal_sample,fig:toy_acausal_sample}.
