...
\renewcommand{\ss}[1]{_\text{#1}}         % Subscripts in text
...
\begin{abstract}
This paper presents the Causal Gaussian Process Convolution Model (CGPCM), a doubly nonparametric model for causal, spectrally complex dynamical phenomena. The CGPCM is a generative model in which white noise is passed through a causal, nonparametric-window moving-average filter, which we show to be equivalent to a Gaussian process with a nonparametric kernel that is biased towards causally-generated signals. We develop enhanced variational inference and learning schemes for the causal and previous acausal variants of the GPCM \citet{Tobar:2015:Learning_Stationary} that significantly improve statistical accuracy. These modelling and inferential contributions are finally demonstrated on synthetic and real-world signals.
\end{abstract}
