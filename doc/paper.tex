...
\end{align*}
where $M^{u}=K_u^{-1}uu^T K_u^{-1}-K_u$. This expression was used to compute the model samples shown in figure 1. %Unlike the GPCM, the kernel approximation $\mathbb{E}[k_{f\cond h}(r)\cond u]$ of the CGPCM will not simply be a mixture of Gaussians; it will be a more complicated expression instead.
...
In \cref{subsec:roughness} we showed that the CGPCM models both smooth and rough signals, whereas the GPCM only models smooth signals. We perform two experiments that show the significance of this modelling capability.
...
The head-related impulse response (HRIR) describes how the reception of sound by the ear depends on the position from which the sound originates. In particular, the received sound is convolved with the HRIR before it is sensed by the ear. We consider the problem of determining the HRIR given some signal sensed by the ear.
